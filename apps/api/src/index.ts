...
app.get("/health", (_req, res) => res.json({ ok: true, service: "hologram-api", version: "0.6.0", assistant: "HOLO", auth: "supabase", time: new Date().toISOString() }));
...
// Member billing/payment endpoints remain protected by Supabase auth.
app.use("/api", requireAuth, directorRouter, paymentsRouter, apiRouter);
