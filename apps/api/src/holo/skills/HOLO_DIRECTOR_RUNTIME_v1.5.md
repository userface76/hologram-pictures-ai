# HOLO Director Runtime Skill v1.5

Status: ACTIVE RUNTIME DISTILLATION

This file is the compact runtime skill pack used by HOLO Director Mode. The larger research/master MD files may keep growing, but runtime should select concise production rules instead of injecting an entire archive into every request.
...
Maintain natural motion and facial consistency.

Luxury fashion often benefits from:
- 50mm or 85mm
- controlled walk/pose
- clean composition
- rim or shaped studio light
- restrained camera movement
- silhouette/fabric motion

## 11. Action choreography

Before visual spectacle, establish geography:
- attacker / responder
- travel direction
- force direction
- camera side
...
Keep screen direction stable across connected beats unless a deliberate transition resets the axis.

## 12. Continuity / identity

Use stable IDs conceptually for recurring assets.
Separate immutable traits from variable behavior.
Track:
...
If start and end conflict (day/night, wardrobe, product state), either explain a plausible transition or flag continuity risk.
...
Voice profile: age character + emotion + timbre + pace/delivery. Keep it concise.

Audio layers:
- dialogue
- ambient
- critical sound event
- music
- intentional silence
...
Motion should match environment: wind moves hair/fabric/leaves; rain affects surfaces; city motion differs from forest motion.

Palette should support emotion and continuity rather than add arbitrary colors.
...
## 18. Two-candidate policy

HOLO Director Mode returns exactly two genuinely different strategies, not paraphrases.

Candidate A — STABLE / CONTROLLED
- continuity and identity/product accuracy first
- simpler camera logic
- reliable motion budget
- recommended when references or short duration create risk

Candidate B — CINEMATIC / IMPACT
- stronger angle/lens/reveal strategy
- more expressive but still feasible camera/motion
- recommended when the idea benefits from drama and the continuity risk is manageable
