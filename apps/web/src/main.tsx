...
function App(){
  ...
  async function analyze(autoRender=false){
    ...
    try{
      ...
      setPlan(d.plan); setStatus(d.job?`렌더 작업 생성됨 · ${d.job.status}`:`분석 완료 · ${d.source==='astra'?'GPT-6 Astra':'로컬 파서'}`);
  ...
  return <main className="space">
    ...
    <header><div><b>HOLOGRAM</b> PICTURES AI</div><small>CONVERSATIONAL AI VIDEO OS · V0.2</small></header>
