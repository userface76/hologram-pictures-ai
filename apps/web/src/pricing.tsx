...
const plans = [
  {
    ...
    credits: "5초 체험 크레딧",
    videos: "5초 영상 1회",
    features: ["MiniMax H3 체험", "이미지 → 영상", "HOLO 프롬프트 분석", "개인 보관함"],
  },
  {
    id: "starter",
    name: "STARTER",
    ...
    price: "29,000원",
    credits: "150초 / 월",
    videos: "10초 기준 최대 15편",
    features: ["MiniMax H3", "이미지 · 텍스트 → 영상", "HOLO 프롬프트", "개인 프로젝트 · 영상 보관함", "상업적 사용"],
  },
  {
    id: "creator50",
    name: "CREATOR 50",
    ...
    credits: "500초 / 월",
    videos: "10초 기준 최대 50편",
    featured: true,
    features: ["MiniMax H3", "이미지 · 텍스트 → 영상", "HOLO 프롬프트", "회원 전용 보관함", "상업적 사용", "우선 렌더링", "런칭 특별가"],
  },
  {
    id: "pro",
    name: "PRO",
    ...
    price: "169,000원",
    credits: "900초 / 월",
    videos: "10초 기준 최대 90편",
    features: ["MiniMax H3", "고용량 영상 생성", "HOLO 프롬프트", "프로젝트 · 영상 보관함", "상업적 사용", "우선 렌더링", "2K 생성 지원"],
  },
];
...
function PricingPage() {
  ...
  function choosePlan(planName: string) {
    ...
    setNotice(`${planName} 선택을 확인했습니다. 결제 모듈 연결 후 바로 구독할 수 있도록 준비 중입니다.`);
  }
  ...
  return (
    <main className="pricingPage">
      ...
      <header className="pricingHeader">
        ...
        <button className="backToHolo" onClick={() => { window.location.hash = "#app"; window.location.reload(); }}>HOLO로 돌아가기</button>
      </header>
        ...
        {plans.map((plan) => (
          <article key={plan.id} className={`planCard ${plan.featured ? "featured" : ""}`}>
            ...
            <div className="planPrice">
              {plan.originalPrice && <del>{plan.originalPrice}</del>}
              <strong>{plan.price}</strong><small>/ 월</small>
            </div>
            <div className="planCredit"><b>{plan.credits}</b><span>{plan.videos}</span></div>
            <ul>{plan.features.map((feature) => <li key={feature}>{feature}</li>)}</ul>
            <button onClick={() => choosePlan(plan.name)}>{isAdmin ? "ADMIN · 제한 없음" : plan.id === "creator50" ? "CREATOR 50 선택" : `${plan.name} 선택`}</button>
          </article>
        ))}
      </section>
      ...
      <p className="pricingFineprint">영상 수량은 10초 · 768P 기준입니다. 실제 사용량은 영상 길이, 해상도, 생성 모델 및 재생성 여부에 따라 달라질 수 있습니다. 결제 기능은 별도 결제 모듈 연결 후 활성화됩니다.</p>
